package com.agro.inventory.util.animatedtext
...
import com.agro.inventory.util.convertDpToPixel
...
internal fun TextView.showProgress(params: ProgressParams) {
    params.apply {
        ...
        val colors = when {
            progressColorRes != null -> intArrayOf(context.color(progressColorRes!!))
...
private fun TextView.showDrawable(
    ...
    val drawableMargin = if (textMarginPx == AnimatedText.DEFAULT) {
        context.convertDpToPixel(DEFAULT_DRAWABLE_MARGIN_DP).toInt()
    } else {
        textMarginPx
    }
...
private fun generateProgressDrawable(
    ...
    return CircularProgressDrawable(context).apply {
        ...
        if (progressRadiusPx != AnimatedText.DEFAULT) {
            centerRadius = progressRadiusPx.toFloat()
        }
        if (progressStrokePx != AnimatedText.DEFAULT) {
